...
export class VideoPost {
    ...
    createHeartButton() {
        ...
        const clickHandler = async (e) => {
            e.preventDefault();
            e.stopPropagation();
            if (!this.favoritesManager || this.isTogglingFavorite)
                return;
            ...
            try {
                ...
                if (newFavoriteState) {
                    ...
                    this.data.marker.tags = this.data.marker.tags.filter(tag => tag.name !== FAVORITE_TAG_NAME);
                }
                this.updateHeartButton(heartBtn);
            }
            catch (error) {
     ...
     */
    handleRatingKeydown(event, currentIndex) {
    ...
    syncRatingDialogLayout() {
        ...
        if (!cardRect.width || !wrapperRect.width)
            return;
        const footer = this.footer || this.container.querySelector('.video-post__footer');
    ...
    async checkFavoriteStatus() {
        if (!this.favoritesManager)
            return;
        try {
            const hasFavoriteTag = this.data.marker.tags?.some(tag => tag.name === FAVORITE_TAG_NAME) || false;
